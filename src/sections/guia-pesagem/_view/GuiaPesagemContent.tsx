...
export function GuiaPesagemContent() {
  return (
    <>
      ...
      <Clause text="1.2. FLUXO E TENTATIVAS NA BALANÇA: O atleta terá direito a uma única aferição oficial caso o peso esteja correto. Caso falhe na primeira tentativa, disporá de uma janela limite pré-estipulada no cronograma oficial para uma segunda e última aferição. São vedadas manobras de apoio estrutural, segurar em equipamentos ou qualquer manipulação física que interfira no sensor de carga." />
      ...
      <Clause text="5.3. JANELA DE AFERIÇÃO E DESCLASSIFICAÇÃO: O sorteado disporá de janela restrita de tempo (antes do início do evento principal) para aferição única. O não comparecimento à chamada, ou a falha na confirmação do peso em tolerância, gerará exclusão imediata via sistema, vetando sua participação no combate." />

      <SectionTitle>CAPÍTULO VI: PENALIDADES POR FALHA DE PESO</SectionTitle>
      <Clause text="6.1. ESTOURO DE PESO NA PESAGEM OFICIAL: A falha definitiva no enquadramento de categoria dentro da janela da pesagem acarretará desclassificação sumária, vitória por W.O. do oponente e perda das garantias contratuais de Bolsa de Luta. Excepcionam-se categorias das classes infantis e juvenis, que, por decisão regulamentar de base, poderão ter remanejamento extraordinário de chaves." />
